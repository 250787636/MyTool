...
func IosBinCheck(c *gin.Context) {
	...
	if iosResponse.State != 200 {
		if iosResponse.Msg == "签名验证失败" || iosResponse.Msg == "token验证失败" {
			...
			log.Error("测评平台用户名和密码设置错误,错误信息为:", iosResponse.Msg)
			response.FailWithMessage("测评平台用户名和密码设置错误", c)
			return
		} else {
...
func CheckIpaInfo(taskId float64, hand *IosHandle) {
	...
	if errMessage != "" {
		if errMessage == "签名验证失败" || errMessage == "token验证失败" {
			log.Error("测评平台用户名和密码设置失败,", errMessage)
			return
		} else {
	...
	ApkSize := reponse["app_size"].(float64)
	...
	if knum-num == 1 {
		...
		if err := app.DB.Model(&model.CePingUserTask{}).Where("task_id = ?", taskId).Updates(&taskInfo).Error; err != nil {
			log.Error("更新ios应用信息失败，错误为", err.Error)
			return
		}
	}
	...
	s := formatFileSize(int64(ApkSize))
...
func GetIpaInfo(taskId float64, hand *IosHandle) {
	...
	if infoNum.State != 200 {
		...
		if infoNum.Msg != "" {
			hand.RemoveTask(int(taskId))
			log.Error("获取ios应用信息失败，错误为", infoNum.Msg)
			return
		}
	}
...
func IosBatcFileDelete(c *gin.Context) {
	...
	for _, id := range idArrayStr {
		iosTaskId, _ := strconv.Atoi(id)
		IosHandler.RemoveTask(iosTaskId)
