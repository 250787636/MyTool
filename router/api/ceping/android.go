...
func BinCheckApk(c *gin.Context) {
	...
	//fmt.Println(adResponse)

	if adResponse.State != 200 {
		if adResponse.Msg == "签名验证失败" || adResponse.Msg == "token验证失败" {
			log.Info("用户名:", app.Conf.CePing.UserName)
			log.Info("密码:", app.Conf.CePing.Password)
			log.Error("调用测评平台上传apk接口失败信息为:", string(post))
			response.FailWithMessage("测评平台用户名或密码设置错误", c)

			return
		} else {
			log.Error("调用测评平台上传apk接口失败信息", string(post))
			response.FailWithMessage("调用测评平台上传apk接口失败", c)
			return
		}
...
func CheckTask(taskId float64, modelInfo model.CePingUserTask) {
	...
	for {
		...
		//fmt.Println("re", reponse)
		//fmt.Println("reponse----", reponse["common_statistic"])
		//var commonInfo map[string]interface{}
		//jsoniter.Unmarshal(reponse["common_statistic"],&commonInfo)

		//fmt.Println("所需检查测评项数", reponse["item_knum"])
		//fmt.Println("已完成测评项数", reponse["item_num"])
		knum, _ := reponse["item_knum"].(float64)
		num, _ := reponse["item_num"].(float64)
		score, _ := reponse["app_score"].(float64)
...
func BatchDownload(c *gin.Context) {
	...
	var TaskInfo []model.CePingUserTask
	if err := app.DB.Model(&model.CePingUserTask{}).Where("task_id in (?)", idArray).Find(&TaskInfo).Error; err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
...
func BatchFileDelete(c *gin.Context) {
	...
	for _, id := range idArrayStr {
		adTaskId, _ := strconv.Atoi(id)
		AndroidHandler.RemoveTask(adTaskId)
...
type GetAllInfoRequest struct {
	...
	TaskType    int    `form:"task_type"` // 1 android 2 ios 3 小程序
	Status      string `form:"status"`    // 检测中 检测完成  检测失败
}
...
func GetAllInfo(c *gin.Context) {
	...
	if err != nil {
		response.FailWithMessage("必传参数缺少", c)
		return
	}
	...
	//1.如果管理员可以查看所有的数据
	//if isAdmin {
	//	sql := app.DB.Model(&model.CePingUserTask{})
	//	var responseAll []map[string]interface{}
	//	if req.TaskType != 0 {
	//		sql = sql.Where("ce_ping_user_task.task_type = ?", req.TaskType)
	//	}
	//	if req.StartTime != "" {
	...
	if isAdmin {
		if req.CreatedName != "" {
			sql = sql.Where("user_info.username like ?", "%"+req.CreatedName+"%")
		}
	} else {
		sql = sql.Where("user_info.id = ?", userId)
	}
	if req.Status != "" {
		sql = sql.Where("ce_ping_user_task.status = ?", req.Status)
	}
	if req.PkgName != "" {
		sql = sql.Where("ce_ping_user_task.pkg_name like ?", "%"+req.PkgName+"%")
	}
	if req.AppName != "" {
		sql = sql.Where("ce_ping_user_task.app_name like ?", "%"+req.AppName+"%")
	}
	if req.TaskType != 0 {
		sql = sql.Where("ce_ping_user_task.task_type = ?", req.TaskType)
	}
	...
	sql.Joins("inner join user_info on ce_ping_user_task.user_id = user_info.id").
		Joins("inner join template on ce_ping_user_task.template_id = template.id").
		Select("ce_ping_user_task.pkg_name," +
			"ce_ping_user_task.app_name," +
			"ce_ping_user_task.version," +
